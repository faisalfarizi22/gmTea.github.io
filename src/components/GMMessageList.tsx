...
const GMMessageList: React.FC<GMMessageListProps> = ({ 
  ...
  // Efek untuk mengambil data pengguna secara batch
  useEffect(() => {
    if (!uniqueAddresses.length) return;
    
    // Filter hanya alamat yang belum kita ambil datanya
    ...
    async function fetchUserData() {
      ...
      // Proses dalam batch untuk menghindari terlalu banyak request
      const batchSize = 3;
      ...
      for (let i = 0; i < addressesToFetch.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async (address) => {
          try {
            // Gunakan hook useUserDataCombined untuk setiap alamat
            // Karena hooks hanya bisa digunakan di level atas komponen,
            // kita perlu menggunakan fetch manual di sini
            ...
            // Tambahkan data default jika gagal
            newUsersData[address] = {
              highestTier: -1,
              username: null,
              tierName: 'None'
        ...
        // Delay kecil di antara batch untuk menghindari rate limiting
                    ...
                    >
                      {(() => {
                        try {
                          ...
                          // Fallback rendering if ChatMessage fails
                          return (
