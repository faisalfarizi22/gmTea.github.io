...
import { FaChartLine, FaLeaf, FaUsers } from 'react-icons/fa';
...
interface StatsCardProps {
  ...
  isLoading: boolean;
  globalCheckinCount: number; // Tambahkan prop untuk total global
}
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="card overflow-hidden">
        ...
        <h3 className="text-sm font-medium text-gray-500 mb-2 relative z-10">Global Check-ins</h3>
          ...
          <div className="flex flex-col items-center relative z-10">
            <div className="relative">
              <div className="w-24 h-24 rounded-full bg-emerald-100 pulse-ring flex items-center justify-center">
                <span className="text-3xl font-bold text-emerald-700">{globalCheckinCount}</span>
              </div>
              <div className="absolute -top-1 -right-1 w-8 h-8 bg-emerald-600 rounded-full flex items-center justify-center shadow-md">
                <span className="text-white font-bold text-xs">🌍</span>
              </div>
            ...
            <p className="mt-1 text-xs text-emerald-600">on Tea Sepolia</p>
          </div>
        )}
      </div>
    </div>
