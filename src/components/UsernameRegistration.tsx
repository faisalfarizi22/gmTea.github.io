...
import { RegistrationType, RegistrationState } from '@/types/badge';
import { registerWithReferral, checkReferrer, getAddressByUsername } from '@/utils/badgeWeb3';
...
const UsernameRegistration: React.FC<UsernameRegistrationProps> = ({
  ...
  const [currentStep, setCurrentStep] = useState<RegistrationStep>('idle' as RegistrationStep);
  ...
  // Check if username is available using contract call
  const checkUsernameAvailability = async (usernameToCheck: string) => {
    ...
    try {
      ...
      const { getUsernameContract, getProvider } = await import('@/utils/badgeWeb3');
      const provider = getProvider();
      
      if (!provider) {
        setUsernameError('Could not connect to blockchain');
        setIsCheckingUsername(false);
        return;
      }
      
      const usernameContract = getUsernameContract(provider);
      const isAvailable = await usernameContract.isUsernameAvailable(usernameToCheck);
  ...
  const checkReferrerValidity = async (referrerToCheck: string) => {
    ...
    try {
      ...
      // Step 1: Check if referrer username exists
      const referrerAddr = await getAddressByUsername(referrerToCheck);
      ...
      // Step 2: Check referrer's current referral count
      const { getReferralContract, getProvider } = await import('@/utils/badgeWeb3');
      const provider = getProvider();
      
      if (!provider) {
        setReferrerError('Could not connect to blockchain');
        return;
      }
      
      const referralContract = getReferralContract(provider);
      
      // Get referral stats to check count
      const stats = await referralContract.getReferralStats(referrerAddr);
      const referralCount = stats.totalReferrals.toNumber();
      
      if (referralCount >= 10) {
        // Referrer has reached the max limit (10)
        setIsReferrerValid(false);
      ...
      // Referrer is valid and has capacity
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-4 rounded-md text-sm font-medium transition-all duration-200 ${
                      registrationState.isLoading 
                        ...
                        : isCheckingUsername || isCheckingReferrer || isUsernameAvailable === false || isReferrerValid === false || !!usernameError || !!referrerError || referrerAtCapacity
